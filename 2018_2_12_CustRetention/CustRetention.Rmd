...
$\mathbf{x_{i,g}}$ vector of predictors ( $i^{th}$ observation in group $g$ ) $i = 1,...,n , g = 1,...,G$
...
$l(\beta)$ log-likelihood: $\sum_{i=1}^{n}\{y_{i}\eta_{\beta}(\mathbf{x_{i}})-log[1+exp(\eta_{\beta}(\mathbf{x_{i}}))]\}$
...
– $sensitivity(z)=\frac{1}{m}\sum_{i=1}^{m}I(X_{i}\geq z)$

– $specificity(z)=\frac{1}{n}\sum_{j=1}^{n}I(Y_{j}<z)$
